...
def generate_news(raw_data_path):
    for mode in ['train', 'dev', 'test']:
        ...
        for line in open(os.path.join(raw_data_path, f'MIND-{args.mode}/train/news.tsv'), encoding='utf-8'):
...
def generate_traindata(raw_data_path, file_num=512):
    data = []
    for line in open(os.path.join(raw_data_path, f'MIND-{args.mode}/train/behaviors.tsv'), 'r', encoding='utf-8'):
...
def generate_testdata(raw_data_path, mode):
    ...
    with open('./data/speedy_data/{}/ProtoBuf_0.tsv'.format(mode), 'w', encoding='utf-8') as f:
        for line in open(os.path.join(raw_data_path, f'MIND-{args.mode}/{mode}/behaviors.tsv'), 'r', encoding='utf-8'):
            impid, uid, time, history, impressions = line.strip().split('\t')
...
def main():
    args = get_arguments()
    assert os.path.exists(os.path.join(args.raw_data_path, f'MIND-{args.mode}/train'))
    assert os.path.exists(os.path.join(args.raw_data_path, f'MIND-{args.mode}/dev'))
    assert os.path.exists(os.path.join(args.raw_data_path, f'MIND-{args.mode}/test'))
